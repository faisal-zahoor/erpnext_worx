...
class WorkOrder(Document):
	...
	def validate(self):
		...
		if self.sales_order:
			self.unit_selling_price = frappe.db.get_value('Sales Order Item', {'parent': self.sales_order, 'item_code': self.production_item}, 'rate')
			self.selling_price = self.unit_selling_price * self.qty

		self.manufacturing_cost = 0
		for item in self.required_items:
			self.manufacturing_cost += item.amount
		if self.scrap_percentage:
			self.manufacturing_cost += self.manufacturing_cost * self.scrap_percentage / 100
		self.profit = self.selling_price - self.manufacturing_cost
